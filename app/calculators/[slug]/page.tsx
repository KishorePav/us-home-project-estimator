...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}){const {slug}=await params;const project=projects.find(item=>item.slug===slug);if(!project)return {};return {title:`${project.title} Cost Calculator`,description:`${project.description} Build a free US planning range and compare contractor quotes.`,alternates:{canonical:`/calculators/${project.slug}`}}}
