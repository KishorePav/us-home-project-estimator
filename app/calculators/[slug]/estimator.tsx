...
const cleanQuoteAmount=(value:string)=>value.replace(/\D/g,"");
const parseQuoteAmount=(value:string)=>Number(value)||0;
...
export function Estimator({project}:{project:Project}){
 ...
 const [quotes,setQuotes]=useState([{name:"Contractor A",amount:""},{name:"Contractor B",amount:""},{name:"Contractor C",amount:""}]);
 ...
 const setQuote=(index:number,key:"name"|"amount",value:string)=>{
  markStarted();
  setQuotes(old=>old.map((q,i)=>i===index?{...q,[key]:value}:q));
  if(key==="amount"&&parseQuoteAmount(value)>0&&!quoted.current.has(index)){
 ...
 const printReport=()=>{
  trackEvent("estimate_report_printed",{calculator_slug:project.slug,calculator_name:project.title,quotes_added:quotes.filter(q=>parseQuoteAmount(q.amount)>0).length,estimate_customized:customized.current});
 ...
 return <section className="estimator-shell"><div className="estimate-form"><div className="form-intro"><span className="step">01</span><div><h2>Describe the project</h2><p>Use approximate measurements for early planning. You can print a revised estimate later.</p></div></div>
  ...
  <section className="quotes"><div><span className="eyebrow">Quote comparison</span><h3>Compare contractor quotes</h3><p>Use comparable scopes—not total price alone.</p></div>{quotes.map((quote,index)=>{const amount=parseQuoteAmount(quote.amount);return <div className="quote-row" key={index}><input aria-label={`Contractor ${index+1} name`} value={quote.name} onChange={e=>setQuote(index,"name",e.target.value)}/><div className="input-prefix"><span>$</span><input aria-label={`${quote.name} amount`} type="text" inputMode="numeric" pattern="[0-9]*" autoComplete="off" value={quote.amount} placeholder="Enter quote" onChange={e=>setQuote(index,"amount",cleanQuoteAmount(e.target.value))}/></div><span className={`quote-status ${amount?(amount<result.low?"low":amount>result.high?"high":"inside"):""}`}>{amount?(amount<result.low?"Below range":amount>result.high?"Above range":"Within range"):"Add quote"}</span></div>})}</section>
